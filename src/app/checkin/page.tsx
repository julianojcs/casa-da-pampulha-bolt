...
async function getCheckinInfo() {
  await dbConnect();
  const items = await CheckinInfo.find({ isActive: true, isRestricted: false }).sort({ order: 1 });
  return JSON.parse(JSON.stringify(items));
}

async function getCheckoutInfo() {
  await dbConnect();
  const info = await CheckoutInfo.findOne({ isActive: true });
...
export default async function CheckinPage() {
  const items = await getCheckinInfo();
      ...
      <section className="bg-amber-50 py-8">
        <div className="container-section py-0">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <Link href="/checkin/cadastro" className="btn-primary">
              Fazer Cadastro
            </Link>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div>
            ...
            <div className="space-y-4">
              ...
              {/* Checkout Instructions from DB */}
              {checkoutInfo && checkoutInfo.instructions && checkoutInfo.instructions.length > 0 && (
                <div className="bg-white p-6 rounded-lg shadow-md mt-6">
                  ...
                  <ul className="space-y-2">
                    {checkoutInfo.instructions.map((instruction: string, index: number) => (
